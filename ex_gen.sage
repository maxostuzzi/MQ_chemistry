...
def _poly_to_m4gb(p, var_names):
    ...
    Powers are expanded as repeated products to avoid
...
# ── Pseudo-Oil Subspace computation ─────────────────────────────────────────────────────────
...
class _TreeNode:
    def __init__(self, label, valid=True, parent=None):
        self.label = label
        self.valid = valid          # constraint satisfied at this node
        self.children = []
        self.on_success_path = False
        self.parent = parent

    def add_child(self, label, valid=True):
        child = _TreeNode(label, valid, parent=self)
        self.children.append(child)
        return child

    def mark_success(self):
        self.on_success_path = True
        if self.parent:
            self.parent.mark_success()

def print_tree(node, prefix="", is_last=True, is_root=True):
    GREEN = "\033[32m"
    DIM   = "\033[2m"
    RESET = "\033[0m"
    if node.on_success_path:
        color = GREEN
    elif not node.valid:
        color = DIM
    else:
        color = ""
    label = f"{color}{node.label}{RESET if color else ''}"
    if is_root:
        print(label)
    else:
    ...
    for i, child in enumerate(node.children):
        print_tree(child, child_prefix, i == len(node.children) - 1, is_root=False)

# ── Structured solver on the m×m Ptilde system ───────────────────────────────

def solve_Ptilde(Ptilde_mm, k, l, bs, q, target=None, verbose=False, _trace=None):
    ...
    # ── Traced variants (only used when _trace is not None) ───────────────────

    def _brute_block_all(j, fixed):
        """Yields (trial, ok, vals) for every brute-force assignment of block j."""
        var_idx = block_var_range(j)
        eq_idx  = block_eq_range(j)
        nv = bs[j]
        for vals in prd(F, repeat=nv):
            ...
            for idx, val in zip(var_idx, vals):
                trial[idx] = val
            polys, _, _ = build_polys(eq_idx, [], trial)
            ok = all(v == F.zero() for v in polys)
            yield trial, ok, vals

    def dfs_traced(j, fixed, parent_node):
        """DFS with tree building. Yields (complete_fixed, leaf_node)."""
        if j < 0:
            sols = list(solve_free(fixed))
            if sols:
                for new_fixed in sols:
                    if free_range:
                        fv_str = '(' + ', '.join(str(new_fixed[idx]) for idx in free_range) + ')'
                    else:
                        fv_str = 'trivial'
                    leaf = parent_node.add_child(f"MQ(m - k - L, m - L) = {fv_str}  ✓", True)
                    yield new_fixed, leaf
            else:
                parent_node.add_child("MQ(m - k - L, m - L): no solution  ✗", False)
            return

        nv = bs[j]
        if Integer(q)**nv <= 2000:
            for trial, ok, vals in _brute_block_all(j, fixed):
                vals_str = ', '.join(str(v) for v in vals)
                status = "✓" if ok else "✗"
                child = parent_node.add_child(f"MQ(b{j+1}, b{j+1}) = ({vals_str})  {status}", ok)
                if ok:
                    yield from dfs_traced(j - 1, trial, child)
        else:
            var_idx = block_var_range(j)
            eq_idx  = block_eq_range(j)
            polys, var_names, _ = build_polys(eq_idx, var_idx, fixed)
            ...
            if sols is None:
                for trial, ok, vals in _brute_block_all(j, fixed):
                    vals_str = ', '.join(str(v) for v in vals)
                    status = "✓" if ok else "✗"
                    child = parent_node.add_child(f"block {j} = ({vals_str})  {status}", ok)
                    if ok:
                        yield from dfs_traced(j - 1, trial, child)
            elif not sols:
                parent_node.add_child(f"block {j}: M4GB → no solution  ✗", False)
            else:
                for sol_dict in sols:
                    new_fixed = dict(fixed)
                    for pos, idx in enumerate(var_idx):
                        new_fixed[idx] = sol_dict[f'z{pos}']
                    vals_str = ', '.join(str(sol_dict[f'z{pos}']) for pos in range(nv))
                    child = parent_node.add_child(f"block {j} = ({vals_str})  ✓  [M4GB]", True)
                    yield from dfs_traced(j - 1, new_fixed, child)
    ...
    for k_vals in prd(F, repeat=k):
        ...
        if _trace is not None:
            k_str = ', '.join(str(v) for v in k_vals)
            k_node = _TreeNode(f"k = ({k_str})")
            _trace.append(k_node)
            for complete_fixed, leaf_node in dfs_traced(p - 1, fixed0, k_node):
                leaf_node.mark_success()
                sol = vector(F, m)
...
def main():
    ...
    SEP = "=" * 62
    print(SEP)
    print("STEP 1 — MQ system  (n=%d, m=%d, q=%d)" % (n, m, q))
    print(SEP)
    ...
    # ── Step 2: Pseudo-Oil subspace computation ──────────────────────────────────────────

    print()
    print(SEP)
    print("STEP 2 — Pseudo-Oil Subspace Computation  (k=%d, l=%d, bs=%s)" % (k, l, bs))
    print(SEP)

    print("\nComputing O0 ...")
    O0 = compute_O0(FF, P, k, l, bs, verbose=False)
    print(f"  dim(O0) = {len(O0)}")
    print("  Basis (rows):")
    print(Matrix(FF, O0))
    ...
    Oi = basis[len(O0):]
    print(f"  dim(Oi extra vectors, beyond O0) = {len(Oi)}")
    if Oi:
        print("  Basis (rows):")
        print(Matrix(FF, Oi))
    else:
        print("  (none beyond O0)")

    print("\nComputing Otilde ...")
    Otilde = compute_Otilde(FF, P, k, l, bs, basis, verbose=False)
    print(f"  dim(Otilde) = {len(Otilde)}")
    if Otilde:
        print("  Basis (rows):")
        print(Matrix(FF, Otilde))

    # ── Step 3: change of basis and Ptilde ────────────────────────────────────

    S           = Matrix(FF, complete(FF, basis + Otilde, n)).T
    Ptilde_full = [uptriag(S.T * Pi * S) for Pi in P]
    Ptilde_mm   = [M[:m, :m] for M in Ptilde_full]

    # ── Step 4: DFS solve with trace ──────────────────────────────────────────
    ...
    # print("STEP 3 — DFS solver on Ptilde  (k=%d, p=%d, dim0=%d)" % (k, p, dim0))
    # print("Variable layout:  y[0..%d] free | blocks %s | k-vars y[%d..%d]"
    #       % (dim0 - 1, bs, m - k, m - 1))
    # print("Equation layout:  eqs[0..%d] block(innermost→outer) | eqs[%d..%d] free"
    #       % (sum(bs) - 1, sum(bs), m - 1))
    print(SEP)
    print()
    print("Legend:  \033[32mgreen\033[0m = successful branch   "
          "\033[2mdim\033[0m = failed branch (constraint not satisfied)")
    print()

    trace = []
    y = next(solve_Ptilde(Ptilde_mm, k, l, bs, q, target=t, _trace=trace), None)

    print("DFS trees (one per k-assignment tried):")
    print()
    for k_node in trace:
        print_tree(k_node)
        print()

    # ── Step 5: lift solution back ─────────────────────────────────────────────

    print(SEP)
    print("STEP 4 — Lifting solution back")
    print(SEP)
    if y is None:
        ...
        ok       = (result == t)
        print(f"  y (in Ptilde coords) = {list(y)}")
        print(f"  x (original coords)  = {list(x)}")
